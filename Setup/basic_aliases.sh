...
#basic aliases
if grep -q "# basic" ~/.bash_aliases; then
    echo "alias for system already exists"
else
    echo '' >>~/.bash_aliases
    echo '# basic' >>~/.bash_aliases
    echo '## Update ##' >>~/.bash_aliases
    echo 'alias update="sudo apt update && sudo apt full-upgrade -y && sudo apt autoremove -y"' >>~/.bash_aliases
fi
...
if grep -q "# location" ~/.bash_aliases; then
    ...
    echo '## Go to notes ##' >>~/.bash_aliases
    echo 'alias notes="cd ~/Notes"' >>~/.bash_aliases
    echo '## Go to home ##' >>~/.bash_aliases
    echo 'alias home="cd ~"' >>~/.bash_aliases
    echo '## Go to root ##' >>~/.bash_aliases
    echo 'alias root="cd /"' >>~/.bash_aliases
    echo '## Go to dorian ##' >>~/.bash_aliases
    echo 'alias dorian="cd ~/Dev/dorian"' >>~/.bash_aliases
fi
